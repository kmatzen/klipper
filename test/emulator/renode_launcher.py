...
def _render_resc(chip, elf_path, pty_path, monitor_port, log_path,
    ...
    return (
        ...
        'renode_hooks.set_adc_bases({adc_bases})"\n'
...
_TICK_DIAG = bool(os.environ.get('BRIDGE_TICK_DIAG'))
...
def _tick_serve_client(cli, monitor_sock, tick_state, stop_evt,
    ...
    while not stop_evt[0]:
        ...
        while b'\n' in buf:
            ...
            if _TICK_DIAG:
                sys.stderr.write(
                    "renode_launcher: tick advance seq=%d target=%.6f"
                    " delta_us=%d virt=%d\n"
                    % (seq, target_s, delta_us, tick_state['virt_us']))
            # Robustness (mechanism (2), TICK_PROTOCOL_DESIGN.md 4.1): never
            # let an exception escape and silently kill this thread - that
            # would leave `cli` open with nobody replying, wedging klippy's
            # lockstep recv at ~0% CPU forever. Log the failure and still
            # reply `done` (no virtual-time progress this quantum, which
            # klippy tolerates and re-requests) so the handshake stays live
            # and a genuine fault surfaces as a timely test failure, never an
            # infinite hang.
            try:
                ...
                sys.stderr.write(
                    ...
                    "(seq=%d target=%.6f); replying done to keep the "
                    "handshake alive:\n%s"
                    % (seq, target_s, traceback.format_exc()))
            _tick_reply_done(cli, tick_state)
            if _TICK_DIAG:
                sys.stderr.write(
                    "renode_launcher: tick done seq=%d virt=%d\n"
                    % (seq, tick_state['virt_us']))
            seq += 1
    try:
        cli.close()
    except OSError:
